...
export const List = () => {
  ...
  const [monthSelected, setMonthSelected] = useState<string>(String(new Date().getDate() + 1))
  const [yearSelected, setYearSelected] = useState<string>(String(new Date().getDate() + 1))
  ...
  return (
    <Container>
      <HeaderContent titulo={title} lineColor={lineColor}>
        <Select option={options} onChange={(e) => {setMonthSelected(e.target.value)}}/>
        <Select option={optionYear}onChange={(e) => {setYearSelected(e.target.value)}}/>
      </HeaderContent>
